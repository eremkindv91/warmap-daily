/**
 * WarMap Daily - LostArmour Source Adapter
 * Role: PRIMARY_REFERENCE_BASEMAP
 * Provides the official reference baseline control map, polygon tracking, and discrepancy analysis.
 * Implements the Unified Source Data schema:
 * { source: 'lostarmour', source_type: 'reference_baseline', retrieved_at, published_at, version, geometry, metadata }
 */
...
export const LOSTARMOUR_KML_BASE_URL = 'https://lostarmour.info/mapinfo/kml';
export const USER_AGENT = 'WarMapDaily-SyncEngine/2.0 (Compatible; LostArmour Integration)';
...
export function readJson(relPath, defaultValue = null) {
  try {
    ...
    console.warn(`[LostArmour Adapter] Error reading ${relPath}:`, e.message);
    return defaultValue;
  }
}
...
export function writeJson(relPath, data) {
  try {
    ...
    console.error(`[LostArmour Adapter] Error writing ${relPath}:`, e.message);
    return false;
  }
}
...
export function validateLostArmourData(geojson) {
  ...
  // Check bounding box sanity (Eastern Europe / Ukraine / Western Russia: lat [44, 56], lon [22, 43])
...
export async function syncLostArmour(targetDate = null) {
  ...
  try {
    console.log(`[LostArmour Adapter] Fetching official KML from LostArmour (${opDate})...`);
    let fetchRes;
    try {
      fetchRes = await fetchLostArmourKml('latest');
    } catch (err) {
      console.warn(`[LostArmour Adapter] Fetch latest.kml failed (${err.message}), trying date snapshot ${opDate}...`);
    ...
    console.log(`[LostArmour Adapter] Successfully synced ${controlPolygons.length} control polygons (${totalControlAreaKm2} km²). Snapshot saved.`);
    ...
    console.error(`[LostArmour Adapter] Live sync error: ${err.message}. Engaging safety fallback to cached data.`);
 ...
 */
export async function getLostArmourUnified(targetDate = null) {
  const opDate = targetDate || new Date().toISOString().slice(0, 10);
  let latestGeo = getLostArmourSnapshot(opDate) || getLostArmourLatest();
  let isCached = true;

  if (!latestGeo) {
    // Attempt live fetch if no cache exists
    const syncRes = await syncLostArmour(opDate);
    latestGeo = getLostArmourLatest();
    isCached = syncRes.isCachedFallback;
  }

  const version = latestGeo?.metadata?.etag ||
    latestGeo?.metadata?.snapshot_date ||
    (latestGeo ? crypto.createHash('sha256').update(JSON.stringify(latestGeo.metadata || {})).digest('hex').slice(0, 16) : 'unknown');

  return createUnifiedSourceData({
    source: 'lostarmour',
    source_type: 'reference_baseline',
    retrieved_at: new Date().toISOString(),
    published_at: latestGeo?.metadata?.snapshot_date || opDate,
    version,
    geometry: latestGeo || { type: 'FeatureCollection', features: [] },
    metadata: {
      source_name_ru: 'Военно-аналитический ресурс LostArmour',
      source_url: 'https://lostarmour.info/map',
      role: 'PRIMARY_REFERENCE_BASEMAP',
      total_control_area_km2: latestGeo?.metadata?.total_control_area_km2 || 0,
      polygons_count: latestGeo?.metadata?.polygons_count || 0,
      lines_count: latestGeo?.metadata?.lines_count || 0,
      last_modified: latestGeo?.metadata?.last_modified || null,
      is_cached_fallback: isCached,
      validation_passed: Boolean(latestGeo)
    }
  });
}

/**
 * LostArmour Adapter Object implementing standard adapter interface
 */
export const lostArmourAdapter = {
  id: 'lostarmour',
  name: 'LostArmour',
  type: 'reference_baseline',
  fetchRaw: fetchLostArmourKml,
  parseAndNormalize: parseAndNormalizeLostArmour,
  validate: validateLostArmourData,
  sync: syncLostArmour,
  getUnified: getLostArmourUnified,
  getLatest: getLostArmourLatest,
  getSnapshot: getLostArmourSnapshot,
  getDiscrepancies: getLostArmourDiscrepancies,
  getComparison: getLostArmourComparison
};
